...
const validateAddressData = (address) => {
    ...
    if (typeof address !== 'object') {
        throw {
            status: 400,
            message: "Address must be an object with addressLine, city, state, country, pincode"
        };
    }
    return true;
}
...
const validateSettingsData = (settings) => {
    ...
    if (settings.speedLimit && (settings.speedLimit < 0 || settings.speedLimit > 300)) {
        throw {
            status: 400,
            message: "Speed limit must be between 0 and 300 km/h"
        };
    }
    if (settings.lowFuelThreshold && (settings.lowFuelThreshold < 0 || settings.lowFuelThreshold > 100)) {
        throw {
            status: 400,
...
exports.update = async (req, res) => {
    try {
        ...
        if (organizationType && !["logistics", "transport", "school", "taxi", "fleet"].includes(organizationType)) {
...
exports.createSubOrganization = async (req, res) => {
    try {
        ...
        const { name, organizationType, email, phone, address, geo, settings, parentOrganizationId } = req.body;
...
exports.createSubOrgWithManager = async (req, res) => {
    try {
        ...
        if (!organizationData || !managerData) {
            return res.status(400).json({
                status: false,
                message: "organizationData and managerData are required"
            })
        }
        ...
        // Check for duplicate email/phone in organization
        const existingOrg = await Organization.findOne({
            $or: [{ email: organizationData.email }, { phone: organizationData.phone }]
        })
        if (existingOrg) {
            return res.status(409).json({
                status: false,
        ...
        const existingUser = await User.findOne({
            $or: [{ email: managerData.email }, { mobile: managerData.mobile }]
        });
        ...
        // Generate hierarchical path
        const path = `${parentOrg.path || ''}/${organizationData.name.toLowerCase().replace(/\s+/g, '-')}`;

        // Get logo from file upload or body
        let logo = null;
        if (req.file) {
            logo = `/uploads/logos/${req.file.filename}`;
        } else if (organizationData.logo) {
            logo = organizationData.logo;
        }

        // Create sub-organization (adminUser will be set after manager creation)
        const subOrganization = await Organization.create({
        ...
        const hashedPassword = await bcrypt.hash(managerData.passwordHash, 10);
        ...
        // Update organization with adminUser
        subOrganization.adminUser = manager._id;
        await subOrganization.save();
        ...
        console.error("Create Sub-Org with Manager Error:", error);
        return res.status(error.status || 500).json({
            status: false,
            message: error.message || "Internal server error",
        });
