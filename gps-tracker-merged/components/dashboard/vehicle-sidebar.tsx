...
import { setSelectedVehicle } from "@/redux/features/vehicleSlice"
        ...
        <div className="flex w-full flex-col border-r border-white/10 bg-slate-950/80 h-full text-slate-100 antialiased">
            <div className="p-3 border-b border-white/10 space-y-3 bg-slate-950/70">
                ...
                <div className="flex items-center justify-between text-[13px] text-slate-400 font-bold uppercase tracking-widest">
                    <div><span className="text-emerald-300">{vehicles.length}</span> Active Units</div>
                    <button
                ...
                {showFilters && (
                    <div className="flex flex-wrap gap-1 animate-in fade-in slide-in-from-top-1 duration-200">
                        {["total", "running", "idle", "stopped", "inactive", "nodata"].map((f) => (
                            <button
                                key={f}
                                onClick={() => setLocalStatusFilter(f as any)}
                            ...
                            {filteredVehicles.map((v, i) => (
                                <div
                                    ...
                                    onClick={() => {
                                        ...
                                        dispatch(setSelectedVehicle(v.id))
                                    }}
                                    className={`group grid gap-0 border-b border-white/5 py-0.5 px-1 text-[13px] hover:bg-emerald-500/10 cursor-pointer items-center transition-all grid-cols-[120px_120px_40px_40px_40px_40px_70px_1fr_60px_50px] ${selectedId === v.id ? 'bg-emerald-500/15 border-l-2 border-l-emerald-400' : 'border-l-2 border-l-transparent'}`}
                                >
                                    ...
                                    <div className="flex justify-center">
                                        <Info className={`h-2.5 w-2.5 ${selectedId === v.id ? 'text-emerald-300' : 'text-emerald-500/20 hover:text-emerald-400'}`} />
                                    </div>
                                </div>
                            ))}
                        </div>
